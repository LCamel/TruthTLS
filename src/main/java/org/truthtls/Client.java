...
public class Client {
    ...
    public void connect() {
        ...
        try {
            // Generate EC key pair first, so we can add transcript data to it
            keys = new Keys();
            ...
            // 跳过前5个字节（TLS record header）并将剩余内容保存到 Keys 的 transcript 中
            ...
            keys.addTranscript(clientHelloTranscript);
            ...
            while (hasMoreData) {
                ...
                if (dis.available() > 0) {
                    ...
                    if (record.type == 22) {
                        ...
                        // 将整個 handshake 记录添加到 Keys 中的 transcript
                        keys.addTranscript(record.data);
                        ...
                        if (handshake.msg_type == Handshake.SERVER_HELLO && handshake.object instanceof ServerHello) {
                            ...
                            for (Extension extension : serverHello.extensions) {
                                if (extension.type == Extension.KEY_SHARE && extension.object instanceof KeyShareEntry) {
                                    ...
                                    // 不再需要同步 transcript，因为我们已经直接使用 Keys 中的 transcript
